...
export async function fetchRevenue() {
  ...
  try {
    ...
    return [
      { month: "Jan", revenue: 546 },
      { month: "Feb", revenue: 340 },
      { month: "Mar", revenue: 566 },
      { month: "Apr", revenue: 764 },
      { month: "May", revenue: 459 },
    ];
  } catch (error) {
...
export async function fetchCardData() {
  try {
    ...
    const numberOfInvoices = 55;
    const numberOfCustomers = 45;
    const totalPaidInvoices = formatCurrency(566);
    const totalPendingInvoices = formatCurrency(564);
